...
test("plays the shared eight-track editorial sampler directly on Creators and Music", async () => {
  ...
  assert.match(previewHook, /const seekBy = useCallback\(\(seconds: number\) => \{[\s\S]{0,100}seekTo\(audio\.currentTime \+ seconds\)/s);
...
test("keeps the connected workspace readable and artist-led", async () => {
  ...
  assert.equal((musicWorkspace.match(/preview\.seekBy\(-10\)/g) ?? []).length, 2, "inline and fixed players should both rewind ten seconds");
  assert.equal((musicWorkspace.match(/preview\.seekBy\(10\)/g) ?? []).length, 2, "inline and fixed players should both advance ten seconds");
  assert.match(musicWorkspace, /preview\.seekBy\(-10\)[\s\S]{0,100}disabled=\{!isActive \|\| !preview\.canSeek\}[\s\S]{0,120}Go back 10 seconds in \$\{track\.title\}/);
  assert.match(musicWorkspace, /className="music-player-play"[\s\S]{0,220}aria-pressed=\{isPlaying\}/);
  assert.match(musicWorkspace, /preview\.seekBy\(10\)[\s\S]{0,100}disabled=\{!isActive \|\| !preview\.canSeek\}[\s\S]{0,120}Go forward 10 seconds in \$\{track\.title\}/);
  ...
  assert.match(musicWorkspaceV13, /\.music-track-table-head,\s*article\.music-track-row\[role="listitem"\]\s*\{[^}]*grid-template-columns:\s*minmax\(230px, \.8fr\) minmax\(410px, 1\.55fr\) minmax\(130px, \.5fr\) 184px/s);
  assert.match(musicWorkspaceV13, /\.music-track-identity > img\s*\{[^}]*width:\s*64px;[^}]*height:\s*64px;[^}]*object-fit:\s*cover/s);
  assert.match(musicWorkspaceV13, /\.music-track-inline-player\s*\{[^}]*grid-template-columns:\s*auto 34px minmax\(120px, 1fr\) 34px/s);
  assert.match(musicWorkspaceV13, /\.music-player-transport\s*\{[^}]*grid-template-columns:\s*34px 44px 34px/s);
  assert.match(musicWorkspaceV13, /\.music-player-skip\s*\{[^}]*width:\s*34px;[^}]*height:\s*34px/s);
  assert.match(musicWorkspaceV13, /\.music-player-play\s*\{[^}]*width:\s*44px;[^}]*height:\s*44px/s);
  ...
  assert.match(musicWorkspaceV13, /\.workspace-audio-player\s*\{[^}]*grid-template-columns:\s*minmax\(190px, 240px\) auto minmax\(260px, 1fr\) auto/s);
  assert.match(musicWorkspaceV13, /\.workspace-player-main\s*\{[^}]*grid-template-columns:\s*52px minmax\(120px, 1fr\)/s);
  assert.match(musicWorkspaceV13, /\.workspace-player-timeline\s*\{[^}]*grid-template-columns:\s*36px minmax\(140px, 1fr\) 36px/s);
  ...
  const v13ReducedMotionStart = musicWorkspaceV13.indexOf("@media (prefers-reduced-motion: reduce)");
  const v13ForcedColoursStart = musicWorkspaceV13.indexOf("@media (forced-colors: active)");
  assert.ok(v13TabletStart >= 0 && v13CompactStart > v13TabletStart && v13PhoneStart > v13CompactStart && v13ReducedMotionStart > v13PhoneStart && v13ForcedColoursStart > v13ReducedMotionStart);
  const v13TabletCss = musicWorkspaceV13.slice(v13TabletStart, v13CompactStart);
  const v13CompactCss = musicWorkspaceV13.slice(v13CompactStart, v13PhoneStart);
  const v13PhoneCss = musicWorkspaceV13.slice(v13PhoneStart, v13ReducedMotionStart);
  const v13ReducedMotionCss = musicWorkspaceV13.slice(v13ReducedMotionStart, v13ForcedColoursStart);
  ...
  assert.match(v13CompactCss, /\.workspace-audio-player\s*\{[^}]*grid-template-areas:[\s\S]{0,140}"transport timeline timeline"/);
  assert.match(v13CompactCss, /\.workspace-player-timeline\s*\{\s*grid-area:\s*timeline/);
  assert.doesNotMatch(musicWorkspaceV13, /\.music-player-wave[^\{]*\{[^}]*display:\s*none/s, "the compact waveform should remain visible at every V13 breakpoint");
  assert.match(v13PhoneCss, /\.music-player-transport\s*\{[^}]*grid-template-columns:\s*40px 46px 40px/s);
  assert.match(v13PhoneCss, /\.workspace-audio-player\s*\{[^}]*grid-template-columns:\s*1fr;[^}]*grid-template-areas:[\s\S]{0,180}"main"[\s\S]{0,60}"transport"[\s\S]{0,60}"timeline"[\s\S]{0,60}"actions"/);
  assert.match(v13ReducedMotionCss, /\.music-player-wave \.music-wave i\s*\{\s*transition:\s*none/);
