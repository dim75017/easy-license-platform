Warning: truncated output (original token count: 61739)
...
test("uses the two-colour Symbiome surface system instead of retired UI palettes", async () => {
  ...
  assert.deepEqual(Object.fromEntries(accentEntries), {
    ...
    "Jazz Lofi": { color: "#8b4a2f", ink: "#fff9f1" …11739 tokens truncated…Css, /\.offer-powered-lockup\s*\{[^}]*display:\s*inline-flex;[^}]*align-items:\s*center;[^}]*gap:\s*\.12em;/s);
...
test("keeps the connected workspace readable and artist-led", async () => {
  ...
  assert.match(musicWorkspace, /function DownloadsLibrary[\s\S]{0,420}className="music-secondary-view music-track-browser music-workspace-view"/, "Downloads should use the same full-width canvas and vertical rhythm as Music");
  ...
  assert.match(musicWorkspace, /view === "downloads" && \([\s\S]{0,180}<DownloadsLibrary[\s\S]{0,180}loadState=\{downloadsLoadState\}[\s\S]{0,160}loadedCount=\{downloadedTracks\.length\}[\s\S]{0,220}onRetry=\{\(\) => setCatalogRetryNonce\(\(value\) => value \+ 1\)\}[\s\S]{0,160}savedCount=\{downloadedTrackIds\.size\}[\s\S]{0,240}trackList=\{downloadedTracks\.length \? renderTrackTable\(downloadedTracks, "Downloaded tracks"\) : null\}/);
  assert.match(musicWorkspace, /view !== "downloads" \|\| !libraryActionsReady \|\| downloadedTrackIds\.size === 0[\s\S]{0,500}const missingSavedIds = \[\.\.\.downloadedTrackIds\]\.filter\(\(trackId\) => !knownIds\.has\(trackId\)\)/, "Downloads should restore saved track metadata independently of the currently loaded catalogue page");
  assert.match(musicWorkspace, /async function loadDownloadedTracks\(\)[\s\S]{0,650}missingTrackRequests\.slice\(index, index \+ 6\)/, "saved downloads should hydrate in bounded request batches");
  assert.match(musicWorkspace, /chunk\.map\(async \(\{ savedId, trackId \}\)[\s\S]{0,240}catalogRequestUrl\(\{ page: 1, pageSize: 1, trackId \}\)/, "saved downloads should use exact-track catalogue requests");
  assert.match(musicWorkspace, /const track = page\.tracks\[0\] \?\? null;[\s\S]{0,100}return track\?\.id === savedId \? track : null/, "saved downloads should reject mismatched exact-track responses");
  assert.match(musicWorkspace, /if \(loadedTracks\.length\) setCatalogKnownTracks\(\(current\) => mergeTrackPages\(current, loadedTracks\)\)/, "restored downloads should merge into known catalogue tracks");
  assert.match(musicWorkspace, /const downloadedTracks = useMemo\(\(\) => catalogKnownTracks\.filter\(\(track\) => downloadedTrackIds\.has\(track\.id\)\)/, "restored Downloads rows should remain available if the general catalogue request fails");
  ...
  assert.match(downloadsLibraryBlock[0], /savedCount > loadedCount[\s\S]{0,180}className="music-track-results-status music-downloads-sync-status" role="status" aria-live="polite"/, "partial saved-download loading should keep a compact status");
  assert.match(downloadsLibraryBlock[0], /Some saved downloads could not be loaded right now\.[\s\S]{0,180}onClick=\{onRetry\}>Retry downloads/, "Downloads should expose an explicit retry if metadata restoration fails");
  assert.match(downloadsLibraryBlock[0], /trackList \?\? \(savedCount === 0 \? <div className="music-empty-library">/, "Downloads should render the shared interactive track list when its track data is loaded");
  assert.doesNotMatch(downloadsLibraryBlock[0], /music-download-list|TrackActionIcon kind="download"|Listening copy/, "Downloads should not fall back to a static non-playable list");
