...
function ActionLink({ action }: { action: EditorialAction }) {
  ...
  if (action.external) {
    return <a className={className} href={action.href} target="_blank" rel="noreferrer" aria-label={action.ariaLabel}><span className="support-button-label">{action.label}</span></a>;
  }
  return <Link className={className} href={action.href} aria-label={action.ariaLabel}><span className="support-button-label">{action.label}</span></Link>;
}
